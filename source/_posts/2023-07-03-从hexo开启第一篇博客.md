...
[博客示例 ：updating.top](https://updating.top)
...
2、域名设置（可选），切换到`gh-pages`分支，在仓库的根目录下创建 CNAME 文件，内容为域名，如下图：
...
2、在 `/source `目录中创建一个文件，命名为 `CNAME `，里边的内容为你的域名。例如我的域名为：https://updating.top 该文件的内容为：updating.top 。
